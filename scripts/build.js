...
function stripProtocol(domain) {
  ...
  if (!/^https?:\/\//i.test(urlString)) {
    urlString = /^localhost(:\d+)?$/i.test(urlString)
      ? `http://${urlString}`
      : `https://${urlString}`;
  }
...
function getPublicBaseUrl() {
  ...
  if (process.env.EXPO_PUBLIC_URL) {
    return stripProtocol(process.env.EXPO_PUBLIC_URL);
  }
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    ...
    manifest.extra.expoClient.hostUri = `${publicHost}/${platform}`;
    manifest.extra.expoGo.debuggerHost = `${publicHost}/${platform}`;
...
async function main() {
  console.log("Building static Expo preview...");
